...
void writing_in_file(const char* file_content,const std::string& filename) {
    ...
    while (file_content != nullptr) {
        ...
        std::string dev_num;
        std::string address;
        sonata_driver sonataDriver;
        file_content = sonataDriver.parse_data(file_content);
...
//            sonata_driver_print printer{dev_name, address, dev_num};//помещает то что нашли суда
            std::cout << sonataDriver;//Пишет в консоль то что пишет в файл
            fout << sonataDriver;//запись в файл результата
//            fout << dev_name << dev_num << address << std::endl;//запись в файл результата (теперь не работает)
